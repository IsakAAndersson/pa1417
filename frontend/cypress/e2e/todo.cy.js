describe("Todo Item Manipulation (Requirement 8)", () => {
    ...
    it("should add a new todo item to a task (R8UC1)", () => {
        ...
        cy.get(".todo-list .todo-item").last().find(".editable").should("contain.text", todoText);

        // Check that the todo item is active (not struck through)
        cy.get(".todo-list .todo-item").last().find(".checker").should("have.class", "unchecked").and("not.have.class", "checked");

        // Verify the input field is cleared
        cy.get('.todo-list li:last-child form input[type="text"]').should("have.value", "");
    });
    ...
    it("should keep the Add button disabled when description is empty (R8UC1 Alternative)", () => {
        ...
        cy.get('.todo-list li:last-child form button[type="submit"]').should("be.disabled");

        // Type something then delete it
        cy.get('.todo-list li:last-child form input[type="text"]').type("Something").clear();
